...
SELECT Physician.Name FROM Physician
    ...
    LEFT JOIN Trained_In ON Undergoes.Procedures = Trained_In.Treatment AND
...
SELECT Physician.Name FROM Physician WHERE Physician.EmployeeID IN (
	SELECT Under.Physician From Undergoes Under WHERE NOT EXISTS (
		SELECT * FROM Trained_In WHERE Trained_In.Treatment = Under.Procedures 
									AND Trained_In.Physician = Under.Physician
	)
);
...
SELECT Physician.Name AS Physician, Procedures.Name AS Procedures, Undergoes.Date, Patient.Name AS Patient
FROM Physician, Procedures, Undergoes, Patient
WHERE Undergoes.Patient = Patient.SSN
	AND Undergoes.Procedures = Procedures.Code
    AND Undergoes.Physician = Physician.EmployeeID
    AND NOT EXISTS (
		SELECT * FROM Trained_In WHERE Trained_In.Treatment = Procedures.Code
									AND Trained_In.Physician = Undergoes.Physician
); -- Might be able to write a query with EXCEPT not in MySQL
...
SELECT Physician.Name FROM Physician
    ...
    LEFT JOIN Trained_In ON Undergoes.Procedures = Trained_In.Treatment AND
		Physician.EmployeeID = Trained_In.Physician
WHERE Trained_In.Treatment IS NOT NULL AND Undergoes.Date > Trained_In.CertificationExpires
GROUP BY Physician.EmployeeID;
...
SELECT Patient.Name AS Patient, Physician.Name AS Physician, Nurse.Name AS Nurse, Appointment.Start, Appointment.End, Appointment.ExaminationRoom, PCP.Name AS PrimaryCarePhysician
FROM Patient, Physician, Physician PCP, Appointment
...
WHERE Appointment.Patient = Patient.SSN
    ...
    AND Patient.PCP = PCP.EmployeeID
    AND Appointment.Physician <> Patient.PCP;    
...
WHERE Undergoes.Patient <> (
	SELECT Stay.Patient FROM Stay WHERE Undergoes.Stay = Stay.StayID
);
...
SELECT ExaminationRoom, Count(*) AS TimesUsed FROM Appointment GROUP BY ExaminationRoom;

-- 8.9 Obtain the names of all patients and their primary care physician, such that the following are true:
SELECT Patient.Name As Patient, Physician.Name as PrimaryPhysician FROM Patient, Physician WHERE Patient.PCP = Physician.EmployeeID

-- The patient has been prescribed some medication by his/her primary care physician.
AND (
	EXISTS (
		SELECT * FROM Prescribes WHERE Prescribes.Patient = Patient.SSN AND Prescribes.Physician = Patient.PCP
	)

-- The patient has undergone a procedure with a cost larger that $5,000
	AND EXISTS ( -- OR EXISTS (
		SELECT * FROM Undergoes, Procedures WHERE Undergoes.Procedures = Procedures.Code
...
-- The patient has had at least two appointments where the nurse who prepared the appointment was a registered nurse.
	AND 2 <= ( -- OR 2 <= (
		SELECT COUNT(AppointmentID) FROM Appointment, Nurse 
        WHERE Appointment.PrepNurse = Nurse.EmployeeID AND Nurse.Registered = 1
	)

-- The patient's primary care physician is not the head of any department.
	AND NOT Patient.PCP IN ( -- OR NOT Patient.PCP IN (
